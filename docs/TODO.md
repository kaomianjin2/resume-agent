...
1. [操作] 按 `docs/job-application-development-tasks.md` 逐项执行 JOB-001 到 JOB-019 → verify: 每个任务均有独立 worktree、implementer 提交、reviewer 结论、最小必要测试和观测证据
   ...
   - [x] JOB-006 多平台采集编排与进度状态 → verify: `rtk uv run pytest -p no:cacheprovider tests/test_gui_runtime.py tests/test_storage.py`：65 passed；reviewer 第四次复审 `Can proceed: Yes`
   - [x] JOB-007 平台风控与人工接管 → verify: `rtk uv run pytest -p no:cacheprovider tests/test_gui_runtime.py`：54 passed；reviewer `可继续`
   - [x] JOB-008 BOSS 直聘只读采集适配器 → verify: `rtk uv run pytest -p no:cacheprovider tests/test_gui_runtime.py tests/test_storage.py`：76 passed；reviewer 复审 `可继续`
   ...
   - [x] JOB-014 确认批次重校验 → verify: `rtk uv run pytest -p no:cacheprovider tests/test_gui_runtime.py tests/test_storage.py`：124 passed；全量 330 passed；reviewer `可继续`
   - [x] JOB-015 BOSS 直聘确认后投递适配器 → verify: `rtk uv run pytest -p no:cacheprovider tests/test_gui_runtime.py tests/test_storage.py`：138 passed；全量 348 passed
