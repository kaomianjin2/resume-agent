...
flowchart TD
    ...
    GUIRuntime --> RevalidationVM[确认批次重校验<br/>revalidate_confirmation_batch<br/>下线/已投递/按钮/JD 变化检查]
    RevalidationVM --> JobAdapterProtocol
    RevalidationVM --> JobRecords[(application_records)]
    RevalidationVM --> State
...
- GUI Runtime Facade 的确认批次重校验入口通过 `revalidate_confirmation_batch()` 在投递前重新校验已确认岗位：依次检查岗位下线、已投递、按钮不可用和 JD 关键字段变化，陈旧岗位写入 `skipped` 或 `duplicate` 并记录原因，通过检查的岗位保持 `approved`，重校验结果保存到 `session_state`。
- 求职平台适配器协议位于 `job_platform_adapters.py`，定义标准岗位对象、搜索请求、平台执行结果、确认投递请求、投递结果、浏览器自动化边界和统一错误类型；协议包含 `is_button_available()` 按钮可用性检查方法；当前包含 BOSS 直聘、拉勾、猎聘只读采集适配器与 fake adapter，为采集编排器、重校验和投递执行器提供共享契约。
