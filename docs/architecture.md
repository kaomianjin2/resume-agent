...
- 求职采集编排器位于 `job_collection.py`，由 GUI Runtime Facade 的 `collect_job_applications()` 和 `retry_failed_job_collection_platform()` 调用；它按平台记录 `started`、`page_collected`、`detail_collected`、`completed`、`failed`、`retrying` 状态，单个平台失败或抛异常不清空其他平台结果，并把进度写入 SQLite `collection_platform_progress` 与 GUI `job_collection_progress` view model。
